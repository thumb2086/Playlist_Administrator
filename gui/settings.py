...
class SettingsWindow:
    ...
    def create_widgets(self):
        ...
        tk.Label(lf_conv, text="M4A 子資料夾:", font=("Microsoft JhengHei", 10)).pack(anchor="w")
        self.spotube_m4a_var = tk.StringVar(value=self.config.get('spotube_m4a_subfolder', 'm4a'))
        tk.Entry(lf_conv, textvariable=self.spotube_m4a_var, font=("Consolas", 9)).pack(fill="x", pady=2)
        
        tk.Label(lf_conv, text="MP3 子資料夾:", font=("Microsoft JhengHei", 10)).pack(anchor="w")
        self.spotube_mp3_var = tk.StringVar(value=self.config.get('spotube_mp3_subfolder', 'mp3'))
        tk.Entry(lf_conv, textvariable=self.spotube_mp3_var, font=("Consolas", 9)).pack(fill="x", pady=2)
        
        tk.Label(lf_conv, text="Conversion Threads:", font=("Microsoft JhengHei", 10)).pack(anchor="w")
        self.spotube_workers_var = tk.StringVar(value=str(self.config.get('spotube_convert_workers', 4)))
        tk.Entry(lf_conv, textvariable=self.spotube_workers_var, font=("Consolas", 9), width=10).pack(anchor="w", pady=2)

        self.prefer_mp3_var = tk.BooleanVar(value=self.config.get('prefer_mp3_playlists', True))
        tk.Checkbutton(lf_conv, text="播放清單優先 MP3", variable=self.prefer_mp3_var, font=("Microsoft JhengHei", 10)).pack(anchor="w", pady=2)
  ...
  📁 mp3/ (轉換後 MP3 檔案)"""
        ...
        if access_token:
            self.spotify_login_btn = tk.Button(lf_spotify_api, text="🚪 登出 Spotify", command=self.spotify_oauth_logout, bg="#FF4444", fg="white", font=("Microsoft JhengHei", 9, "bold"))
        else:
        ...
        def _oauth_thread():
            try:
                ...
                if success:
                    ...
                    self.top.after(0, lambda: self._update_login_button_state(True))
    ...
    def spotify_oauth_logout(self):
        """Handle Spotify OAuth logout"""
        if messagebox.askyesno("確認登出", "確定要登出 Spotify 嗎？"):
            # Clear tokens from config
            self.config['spotify_access_token'] = None
            self.config['spotify_refresh_token'] = None
            from utils.config import save_config
            save_config(self.config)
            
            # Update UI
            self.spotify_login_status.config(text="尚未登入", fg="#999999")
            self._update_login_button_state(False)
            messagebox.showinfo("登出", "已登出 Spotify")
    
    def _update_login_button_state(self, is_logged_in):
        """Update login button based on login state"""
        if is_logged_in:
            self.spotify_login_btn.config(text="🚪 登出 Spotify", command=self.spotify_oauth_logout, bg="#FF4444", fg="white")
        else:
            self.spotify_login_btn.config(text="🔐 登入 Spotify", command=self.spotify_oauth_login, bg="#1DB954", fg="white")
