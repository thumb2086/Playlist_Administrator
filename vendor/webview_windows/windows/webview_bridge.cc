...
void WebviewBridge::HandleMethodCall(
  ...
  // getCookies: string url -> raw CDP JSON {"cookies":[...]}
  if (method_name.compare("getCookies") == 0) {
    if (const auto url = std::get_if<std::string>(method_call.arguments())) {
      std::shared_ptr<flutter::MethodResult<flutter::EncodableValue>>
          shared_result = std::move(result);
      webview_->GetCookies(
          *url,
          [shared_result](const std::string& json) {
            shared_result->Success(flutter::EncodableValue(json));
          });
      return;
    }
    return result->Error(kErrorInvalidArgs);
