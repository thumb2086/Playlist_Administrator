...
void Webview::GetCookies(
  ...
  // ICoreWebView2CallMethodCompletedHandler (COM, WRL style)
  auto on_complete = Microsoft::WRL::Callback<
      ICoreWebView2CallDevToolsProtocolMethodCompletedHandler>(
      [callback](HRESULT errorCode, PCWSTR resultObjectAsJson) -> HRESULT {
        if (resultObjectAsJson) {
          const int len = ::WideCharToMultiByte(
              CP_UTF8, 0, resultObjectAsJson, -1, nullptr, 0, nullptr, nullptr);
          std::string out(len > 0 ? len - 1 : 0, '\0');
          if (len > 0) {
            ::WideCharToMultiByte(CP_UTF8, 0, resultObjectAsJson, -1,
  ...
  webview_->CallDevToolsProtocolMethod(L"Network.getAllCookies", L"{}",
                                       on_complete.Get());
}
